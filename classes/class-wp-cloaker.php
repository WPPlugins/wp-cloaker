...
require_once(wp_cloaker_path.'classes/Add_Taxonomy_To_Post_Permalinks.php');
...
class WP_Cloaker{
	...
	public function enqueue_scripts(){
		...
		if( 'wp_cloaker_link_page_wp_cloaker_link_reports' == $page->id ){
			wp_enqueue_script('gchart1','https://www.gstatic.com/charts/loader.js',array('jquery'),wp_cloaker_version);
			wp_enqueue_script('wp-cloaker-reports',wp_cloaker_url.'/assets/js/reports.js',array('jquery'),wp_cloaker_version);
		}
	...
	public function wp_cloaker_link_init(){
		...
		$exclude_cat = get_option('wp_cloaker_link_exclude_cat','no');
		if($exclude_cat == 'no'){
			$rewrite = array(
	...
	public function wp_cloaker_link_options_display(){
		require_once(wp_cloaker_path.'assets/views/link-options.php');	
	}
	//save link options data
	...
	function wp_cloaker_link_template($single){
		 global $wp_query, $post;
		/* Checks for single template by post type */
		if ($post->post_type == "wp_cloaker_link"){
			if(file_exists(wp_cloaker_path . 'assets/views/single-wp_cloaker_link.php'))
				return wp_cloaker_path . 'assets/views/single-wp_cloaker_link.php';
		}
		return $single;
	}
	...
	public function wp_cloaker_link_clicks_details(){
		require_once(wp_cloaker_path.'assets/views/clicks_details.php');
	}
	
	public function wp_cloaker_delete_click_data($postID){
		global $wpdb;
		global $post_type;   
    if ( $post_type != 'wp_cloaker_link' ) return;
